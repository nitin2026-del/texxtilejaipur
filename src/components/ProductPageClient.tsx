...
export function ProductPageClient({ product, relatedProducts, initialReviews, ugcVideos = [], siblingProducts = [] }: { product: any, relatedProducts: any[], initialReviews: any[], ugcVideos?: any[], siblingProducts?: SiblingProduct[] }) {
  ...
  const mediaItems = useMemo(() => {
    if (!product) return [];
    const items = (product.images || []).map((url: string) => ({ type: url?.match(/\.(mp4|webm|mov|ogg)$/i) ? 'video' : 'image', url: url }));
                          ...
                          <div className="flex-1 min-w-0 flex flex-col gap-2 justify-center">
                            ...
                            {product.details.ad_showcase.linked_products.map((lp: {id:string;name:string;image:string}) => (
                              <a
                                key={lp.id}
                                href={`/product/${lp.id}`}
                                className="flex min-w-0 items-center gap-2 sm:gap-3 bg-zinc-50 hover:bg-zinc-100 border border-zinc-200 hover:border-zinc-300 rounded-xl px-2 py-2 sm:px-3 sm:py-2.5 transition-all group"
                              >
                                {lp.image && (
                                  <img
                                    ...
                                    alt={lp.name}
                                    loading="lazy"
                                    className="w-9 h-9 sm:w-11 sm:h-11 rounded-lg object-cover flex-shrink-0 border border-zinc-200"
                                  />
                                )}
                                <div className="flex-1 min-w-0">
                                  <p className="text-[10px] sm:text-[11px] font-bold text-zinc-800 truncate leading-tight">{lp.name}</p>
                                  <p className="text-[9px] sm:text-[10px] text-zinc-400 mt-0.5">Tap to view →</p>
                                </div>
                                <svg className="h-3 w-3 sm:h-4 sm:w-4 text-zinc-400 group-hover:text-zinc-600 group-hover:translate-x-0.5 transition-all flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
