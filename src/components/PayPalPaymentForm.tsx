...
export const PayPalPaymentForm: React.FC<PayPalPaymentFormProps> = ({
  ...
  const cardButtonRendered = useRef(false);

  const usdAmount = Number(amount.toFixed(2));

  // Load the PayPal JS SDK script (only needed for the card button)
  useEffect(() => {
    if (window.paypal) {
      setSdkReady(true);
      return;
  ...
  // Render only the Card button via SDK (no popup needed - uses inline form)
  useEffect(() => {
    if (!sdkReady || !window.paypal || cardButtonRendered.current) return;
    if (!cardRef.current) return;

    cardButtonRendered.current = true;
    ...
    // Render only Card button via SDK
    window.paypal
      .Buttons({
  ...
  // PayPal button uses REDIRECT (not popup) — works perfectly on mobile
  const handlePayPalRedirect = async () => {
    try {
      ...
      if (data.approvalUrl) {
        // Full redirect — no popup, works on ALL mobile browsers
  ...
  return (
    <div className="space-y-4">
      ...
      {/* PayPal Redirect Button — works on all devices */}
      <button
        onClick={handlePayPalRedirect}
        disabled={paypalLoading}
        className="w-full py-3 rounded-xl font-bold text-zinc-900 text-sm flex items-center justify-center gap-3 transition-all disabled:opacity-70"
      ...
      >
        {paypalLoading ? (
          ...
          <>
            {/* PayPal Logo */}
            <svg width="80" height="20" viewBox="0 0 800 200" xmlns="http://www.w3.org/2000/svg">
              <text x="0" y="160" fontFamily="Arial" fontWeight="bold" fontSize="160" fill="#003087">Pay</text>
              <text x="290" y="160" fontFamily="Arial" fontWeight="bold" fontSize="160" fill="#009cde">Pal</text>
            </svg>
          </>
        )}
      </button>
      ...
      {/* SDK Error */}
      {sdkError && (
        <div className="text-center text-red-400 text-xs py-3 border border-red-800 rounded-lg bg-red-900/20">
          Failed to load card options. Please use PayPal above.
        </div>
      )}
